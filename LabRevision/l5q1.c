...
ListNode* HashSearch(HashTable Q3Hash, int key)
{
    ...
    HashTableNode *table = Q3Hash.Table;
    ListNode* cur = table[hashKey].head;
...
int HashInsert(HashTable* Q3HashPtr, int key)
{
    ...
    if(Q3HashPtr->Table[hashKey].size == 0)
    {
        node->next = NULL;
        node->key = key;
        Q3HashPtr->Table[hashKey].head = node;
    ...
    {
        node->next = Q3HashPtr->Table[hashKey].head;
        node->key = key;
        Q3HashPtr->Table[hashKey].head = node;
        Q3HashPtr->Table[hashKey].size++;
        Q3HashPtr->nSize++;
