...
int isBSTX(BTNode *root, BTNode* minN, BTNode* maxN);
...
int isBSTX(BTNode *root, BTNode* minN, BTNode* maxN)
{
    ...
    if(minN!= NULL && root->item <= minN->item) return 0;
    if(maxN!= NULL && root->item >= maxN->item) return 0;

    return isBSTX(root->left,minN,root) && isBSTX(root->right,root,maxN);
