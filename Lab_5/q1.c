...
{
    //write your code here
    int tableKey = Hash(key,Q3HashPtr->hSize);

    HashTable* hashTable = Q3HashPtr;

    if(hashTable->nSize != 0)
    {
        HashTableNode* tableNodeArray = hashTable->Table;
        HashTableNode tableNode = tableNodeArray[tableKey];
        ListNode* tableNodeHead = tableNode->head;

        while(tableNodeHead != NULL)
        {
            if(tableNodeHead->key == modNum)
            {
                getKey = tableNodeHead->key;
            }
            tableNodeHead = tableNodeHead->next;
        }
        tableNodeHead = tableNode->head;
        if(getKey < 0)
        {
             ListNode* newTableNode = (ListNode*) malloc(sizeof(ListNode));
             newTableNode->next = NULL;
             newTableNode->key = key;
            if(tableNodeHead == NULL)
            {
                tableNodeHead = newTableNode;
            }
            else
            {
                tableNodeHead->next = newTableNode;
            }
        }
        else
        {
