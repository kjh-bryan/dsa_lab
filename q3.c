...
int HashDelete(HashTable* Q3HashPtr, int key)
{
    ...
    if(HashSearch(*Q3HashPtr,key) == NULL || Q3HashPtr->hSize == 0)
    {
        return 0;
    }
    else
    {
        node = HashSearch(*Q3HashPtr,key);
    }

    index = Hash(key,Q3HashPtr->hSize);

    if(key == Q3HashPtr->Table[index].head->key)
    {
        Q3HashPtr->Table[index].head = node->next;
        free(node);
        node = NULL;
        return 1;
    }
